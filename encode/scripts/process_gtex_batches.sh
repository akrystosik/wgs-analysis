#!/bin/bash
#/mnt/czi-sci-ai/intrinsic-variation-gene-ex/wgs/scripts/process_gtex_batches.sh
# Script to process GTEx samples in batches
# Usage: ./process_gtex_batches.sh [batch_size] [max_parallel] [start_index]

# Settings with default values
BATCH_SIZE=${1:-50}      # Number of samples per batch
MAX_PARALLEL=${2:-32}    # Number of parallel processes
START_INDEX=${3:-30}     # Start index (skip previously processed samples)
...
ALL_SAMPLES_FILE="${OUTPUT_DIR}/all_samples.txt"
...
# Get all sample IDs if not already saved
if [ ! -f "$ALL_SAMPLES_FILE" ]; then
    echo "Retrieving all sample IDs..."
    bcftools query -l "$VCF" > "$ALL_SAMPLES_FILE"
fi
...
if [ ! -f "$SUMMARY_FILE" ]; then
    ...
    # If there's an existing summary, append it
    if [ -f "${OUTPUT_DIR}/gtex_summary.tsv" ]; then
        grep -v "AVERAGE" "${OUTPUT_DIR}/gtex_summary.tsv" | tail -n +2 >> "$SUMMARY_FILE"
        echo "Loaded existing summary data"
    fi
fi
