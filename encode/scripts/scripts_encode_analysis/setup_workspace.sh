...
mkdir -p "${BASE_DIR}"/{scripts,logs,configs}
mkdir -p "${BASE_DIR}/data/variants"/{deepsomatic,deepvariant,cell_line_analysis_v3}
...
for cell_line in K562 HepG2 Caki2; do
    echo "Setting up directory structure for ${cell_line}..."
    mkdir -p "${BASE_DIR}/data/variants/cell_line_analysis_v3/${cell_line}"/{germline,somatic,final,logs}
done

# Create a helpful README with screen usage instructions
cat > "${BASE_DIR}/README.md" << 'EOF'
# Variant Analysis Pipeline Environment
