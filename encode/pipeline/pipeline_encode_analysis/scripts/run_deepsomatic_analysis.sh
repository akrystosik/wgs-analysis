...
log_message "Starting DeepSomatic analysis for ${SAMPLE_ID} with ${NUM_SHARDS} shards"
log_message "Using CPU-only mode with controlled resource utilization"
...
DEEPSOM_EXIT=$?
if [ $DEEPSOM_EXIT -ne 0 ]; then
    log_message "WARNING: DeepSomatic exited with non-zero status: $DEEPSOM_EXIT"
    # Cleanup zombie processes if any
    log_message "Cleaning up any zombie processes"
    pkill -9 -f "make_examples_somatic" || true
fi

# Record completion
log_message "DeepSomatic analysis completed with exit code: $DEEPSOM_EXIT"
